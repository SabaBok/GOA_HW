...
export default function Header() {
	...
	const [adminLogged] = useState(accs.some(el => el.title == 'admin' && el.logged))
	const adminMode = accs.find(el=> el.title == 'admin' && el.logged)
	//let { adminMode, toggleAdminMode } = useContext(FoodItems)
	...
	function logOut() {
		const updated = accs.map(acc => {
			if (acc.logged) {
				if (acc.title === 'user') {
					return { ...acc, logged: false, cart: [] }
				}
				if (acc.title === 'admin') {
					return { ...acc, logged: false }
				}
			}
			return acc
		})
		...
		<header className='max-md:px-5 w-full z-3 fixed top-0 shadow-[0_2px_8px_rgba(0,0,0,0.12)] bg-white text-black flex justify-between items-center px-5 xl:px-30 py-5 flex-wrap gap-6 border-b border-[#a1a1a131]'>
			...
			<nav className='max-lg:hidden'>
				<ul className='flex max-md:gap-3 gap-15 text-[21px] items-center font-sansita-swashed'>
					{!adminMode && <a href='#hero' className='cursor-pointer relative before:content-[""] before:w-0 before:h-0.5 before:duration-300 before:absolute before:left-1/2 before:bottom-0 before:-translate-x-1/2 before:bg-black hover:before:w-full'>Home</a>}
					{!adminMode && <a href='#order' className='cursor-pointer relative before:content-[""] before:w-0 before:h-0.5 before:duration-300 before:absolute before:left-1/2 before:bottom-0 before:-translate-x-1/2 before:bg-black hover:before:w-full'>Order</a>}
					{!adminMode && <a href='#contact' className='cursor-pointer relative before:content-[""] before:w-0 before:h-0.5 before:duration-300 before:absolute before:left-1/2 before:bottom-0 before:-translate-x-1/2 before:bg-black hover:before:w-full'>Contact</a>}
					...
					{/* admin stuff */}
					{adminMode && (<Link to="/finance" className='cursor-pointer relative before:content-[""] before:w-0 before:h-0.5 before:duration-300 before:absolute before:left-1/2 before:bottom-0 before:-translate-x-1/2 before:bg-black hover:before:w-full'>Finance</Link>)}
					{adminMode && (<Link to="/Kitchen" className='cursor-pointer relative before:content-[""] before:w-0 before:h-0.5 before:duration-300 before:absolute before:left-1/2 before:bottom-0 before:-translate-x-1/2 before:bg-black hover:before:w-full'>Kitchen</Link>)}
				</ul>
			</nav>
			...
			<div className='flex items-center gap-7 max-lg:hidden'>
				...
				{/*{adminLogged && <button onClick={() => toggleAdminMode()} className={`${adminMode ? 'bg-gray-400 text-white' : ''} duration-200 hover:bg-gray-600 hover:text-white rounded-lg px-3 py-1 cursor-pointer`}>admin mode</button>}*/}
			</div>
