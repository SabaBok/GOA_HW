...
export default function Product() {
	...
	return (
		<main className='px-[120px] pb-[120px]'>
			{/* product */}
			<div className='flex flex-col gap-10'>
				...
				<div className='flex justify-between'>
					<div className='flex gap-8 min-h-[530px] flex-1'>
							...
							<div className='flex gap-4'>
								{
									product.sizeList.map((el, id) => (
										<p key={id} className={`duration-150 cursor-pointer ${currSize == el ? 'bg-black text-white' : 'bg-[#F0F0F0] text-[#00000099]'} rounded-full px-5 py-1 `} onClick={() => setCurrSize(el)}>{el}</p>
									))
								}
							</div>
						</div>
