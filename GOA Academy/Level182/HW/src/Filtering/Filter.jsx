...
export default function Filter() {
	...
	useEffect(() => {
		...
		const filtered = data.filter(item => {
			...
			if (!(filters.price[0] < item.price && filters.price[1] > item.price)) return false
			return true
		})
	...
	useEffect(()=>{ if(filters.price[1] == 0) setFilters({ ...filters, price: [filters.price[0], 100000] }) },[filters.price[1]])
