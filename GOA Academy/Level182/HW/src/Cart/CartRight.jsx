...
export default function CardRight({ cartProd }) {
	...
	useEffect(()=>finalPrice<0?setFinalPrice(0):setFinalPrice(prev=>prev),[finalPrice])
	return (
		<div className='border border-[#0000001A] rounded-[20px] px-[24px] py-[20px] h-[440px] flex-5 flex flex-col gap-5'>
			...
			<div className='flex flex-col gap-5'>
				...
				<div className='flex justify-between items-center gap-2'>
					<p className='text-[#00000099]'>Discount {cartProd.length > 0 ? `(-${Math.round((discount/fullPrice)*100)}%)` : ''}</p>
